...
const incomeTypeSchema = z.object({
  name: z.string().min(1, "Name is required"),
  description: z.string().optional(),
});

export default function EditIncomeTypeModal({ incomeType, onClose }) {
  ...
  const onSubmit = async (data) => {
    try {
      setLoading(true);
      await api.put(
        `${GlobalConfig.apiUrl}/v1/income-types/${incomeType.id}`,
        data
      );
      alert("Income type updated successfully!");
      reset();
      onClose(true);
    } catch (error) {
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      alert("Income type deleted successfully!");
      reset();
  ...
  return (
    <Modal onClose={onClose}>
      ...
      >
        <h2 className="text-lg font-bold">Edit Income Type</h2>
