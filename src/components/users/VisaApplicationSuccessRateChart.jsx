...
export default function VisaApplicationSuccessRateChart() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await api.get(
        `${GlobalConfig.apiUrl}/v1/dashboard/users/visa-application-success-rate`
      );
      setSuccessRateData(res.data);
    } catch (error) {
      console.error("Error fetching visa success rate data:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="card border border-base-300 bg-base-100/50">
      <div className="card-body">
        <h2 className="card-title">Visa Application Success Rate</h2>
        {loading ? (
